  ...
  useEffect(() => {
    ...
    async function loadNote() {
      try {
        ...
        if (!hash) throw new Error('Falta la clave de descifrado en el enlace.');

        const response = await fetch(`/api/notes/${encodeURIComponent(id)}`, { cache: 'no-store' });
        const data = await response.json();
        if (!response.ok) throw new Error(data.error || 'La nota no existe o ya ha sido leida.');

        const key = await deriveKey(base64UrlToBytes(hash), base64ToBytes(data.salt));
  ...
  return (
    <main>
      <section className="card">
        ...
        <p style={{ marginTop: 20 }}><Link href="/">Crear otra nota</Link></p>
      </section>
    </main>
  );
}
